...
class VideoProxyChannel(private val context: Context, messenger: BinaryMessenger) :
        ...
        scope.launch {
            jobMutex.withLock {
                ...
                try {
                    ...
                    Log.e(TAG, "Failed to start proxy job", error)
                    jobs.remove(jobId)
                    respondError(result, "transcode_failed", error.message ?: "Transcode failed", recoverable = true)
                }
            }
        }
    }
    ...
    private fun handleCancel(call: MethodCall, result: MethodChannel.Result) {
        val args = call.arguments as? Map<*, *>
        val jobId = args?.get("jobId")?.toString().takeUnless { it.isNullOrBlank() }
        if (jobId == null) {
        ...
        scope.launch {
            val job = jobMutex.withLock { jobs.remove(jobId) }
            job?.future?.cancel(true)
            try {
                job?.outputFile?.deleteRecursively()
            } catch (_: Throwable) {
            }
    ...
    private fun createOutputFile(directory: File): File {
        ...
        return File.createTempFile(prefix, ".mp4", directory)
    }
